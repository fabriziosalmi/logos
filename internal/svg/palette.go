...
// BaseColors are the original 8 primary colors.
...
// CreativeColors are the extended creative palette.
...
// TailwindColors is the full Tailwind CSS v3 color palette (286 colors).
// {hue}-{shade} naming: slate-50 through rose-950.
...
// AllColors merges all palettes.
var AllColors map[string]string

func init() {
	total := len(BaseColors) + len(CreativeColors) + len(TailwindColors)
	AllColors = make(map[string]string, total)
...
// ColorCount returns the total number of named colors.
func ColorCount() int {
